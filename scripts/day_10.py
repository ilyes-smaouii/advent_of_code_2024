...
le_input_table = helpers.get_file_content_as_table(le_filename, lambda x : int(x))
...
def count_trails_from_trailhead_aux(table, curr_pos, curr_distance = 0, curr_trail = []) :
  tt_set = set()
  helpers.print_log_entries("count_trails_from_trailhead_aux() - curr_trail : " + \
    str(curr_trail), log_cats = {"DIRECT"})
  row_count = len(table)
  col_count = len(table[0])
  x, y = curr_pos
  helpers.print_log_entries("count_trails_from_trailhead_aux() - x, y, row_count, col_count :", \
  ...
  if curr_pos == (-1, -1) :
    ...
    if y < col_count - 1 :
      right_pos = (x, y + 1)
    tt_set |= count_trails_from_trailhead_aux(table, up_pos, curr_distance + 1, curr_trail + [(table[x][y], (x,y))])
    tt_set |= count_trails_from_trailhead_aux(table, down_pos, curr_distance + 1, curr_trail + [(table[x][y], (x,y))])
    tt_set |= count_trails_from_trailhead_aux(table, left_pos, curr_distance + 1, curr_trail + [(table[x][y], (x,y))])
    tt_set |= count_trails_from_trailhead_aux(table, right_pos, curr_distance + 1, curr_trail + [(table[x][y], (x,y))])
  helpers.print_log_entries("count_trails_from_trailhead_aux() - returning : " + str(tt_set), log_cats = {"DIRECT"})
  return tt_set

def count_trails_from_trailhead(table, trailhead_pos) :
  return len(count_trails_from_trailhead_aux(table, trailhead_pos, 0, []))

helpers.LOG_DICT["DIRECT"] = (False, "[DIRECT]")

def count_trails_from_all_trailhead(table) :
  total_count = 0
  for tailhead in find_trailheads(helpers.SimpleTableView(le_input_table)) :
    helpers.print_log_entries("Now counting for tailhead " + str(tailhead), log_cats = {"DIRECT"}) # [debugging]
    to_add = count_trails_from_trailhead(table, tailhead)
    helpers.print_log_entries("Found " + str(to_add), log_cats = {"DIRECT"}) # [debugging]
    total_count += to_add
  return total_count

helpers.print_log_entries("Direct approach count : " + str(count_trails_from_all_trailhead(le_input_table)), log_cats={"R"})
######
# PART 2
######