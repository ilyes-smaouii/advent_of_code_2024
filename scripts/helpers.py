...
class SimpleTableView() :
  ...
  def __init__ (self, table, initial_pos = (0, 0)) :
    ...
    if type(table) is list :
      print_log_entries("SimpleTableView.__init__() : got list argument", log_cats = {"D"})
      self._table_rep = table
    elif type(table) is SimpleTableView :
      print_log_entries("SimpleTableView.__init__() : got SimpleTableView argument", log_cats = {"D"})
      self._table_rep = table._table_rep
      print_log_entries("SimpleTableView.__init__() : used SimpleTableView._table_rep", log_cats = {"D"})
      pass
    else :
  ...
  def get_pos_and_cell(self) :
    pos, cell = (self._row_pos, self._col_pos), self.get_cell_at((self._row_pos, self._col_pos))
    return (pos, cell)
  #
  def laxist_set_pos (self, new_pos) :
    print_log_entries("set_pos() - called with " + str(new_pos), log_cats = {"D"}) # [debugging]
    self._row_pos = min(self._row_count - 1, max(0, new_pos[0]))
    self._col_pos = min(self._col_count - 1, max(0, new_pos[1]))
    print_log_entries("set_pos() - returning " + str(self.get_pos()), log_cats = {"D"}) # [debugging]
  ...
  def set_pos (self, new_pos) :
    ...
    if 0 <= new_pos[0] <= self._row_count - 1 and 0 <= new_pos[1] <= self._col_count :
      ...
      raise Exception("SimpleTableView.set_pos() error : invalid position (" + str(new_pos) + ")")
    print_log_entries("set_pos() - returning " + str(self.get_pos()), log_cats = {"D"}) # [debugging]
    return self.get_pos_and_cell()
  #
  def move_up (self) :
  ...
  def __next__ (self) :
    ...
    if self._pos[1] >= self._table_size[1] - 1 :
      ...
      if self._pos[0] >= self._table_size[0] - 1 :
        ...
        self._pos, self._viewed_cell = self._simple_table_view.set_pos((self._pos[0] + 1, 0))
    else :
